...
def centroid_batch(bboxes1: np.ndarray, bboxes2: np.ndarray, w: int, h: int) -> np.ndarray:
    ...
    """
    # TODO
    centroid1 = np.column_stack(((bboxes1[:, 0] + bboxes1[:, 2]) / 2,(bboxes1[:, 1] + bboxes1[:, 3]) / 2) )
    centroid2 = np.column_stack(((bboxes2[:, 0] + bboxes2[:, 2]) / 2,(bboxes2[:, 1] + bboxes2[:, 3]) / 2) )
    centroid1_normalized = centroid1 / np.array([w, h])
    centroid2_normalized = centroid2 / np.array([w, h])
    normalized_distances = np.linalg.norm(centroid1_normalized[:, np.newaxis, :] - centroid2_normalized, axis=2)
...
class Tracker:
    ...
    def assign_detections_to_tracks(self, distances):
        ...
        for i, j in zip(row_ind, col_ind):
            if distances[i, j] < self.max_iou_distance:
...
def count_position_exchanges(csv_file):
    ...
    with open(csv_file, mode='r') as file:
        ...
        for row in reader:
            ...
            previous_tracks[track_id] = track_id
...
if __name__ == "__main__":
    ...
    video_path =  '/home/charlie/HCC/State Estimation Lab-20240516/lab3/track_1.MOV'
    main(video_path)
